...
### الرسم البياني للعلاقات (مُحدَّث بعد دمج الكيانات الناقصة)
...
## دمج الكيانات الناقصة في الـ ERD (تم التنفيذ)

تم دمج جميع العناصر الناقصة في النسخة الحالية من الـ ERD كما يلي:

- **User**: أصبح جدولًا رئيسيًا يربط كل الحقول `approved_by`, `officer`, `responsible`, `uploaded_by`.
- **ManagementActivity (Tool 3)**: مضاف لالتقاط الإجراءات التشغيلية مع `potential_impact`, `recommended_actions`, ومسؤولية المتابعة.
- **MitigationPlan (Tool 4)**: مضاف للخطة الاستراتيجية مع تأثيرات بيئية/اجتماعية/مناخية، جداول زمنية (قبل/أثناء/بعد)، وخيار `is_continuous`.
- **Attachment**: نظام مرفقات عام لأي كيان (`project`, `screening`, `assessment`, `monitoring`) مع تتبع حجم/نوع الملف ورافعه.
- **حقول مضافة للجداول الحالية**:
  - `Screening`: `screening_date`, `status`, أختام زمنية.
  - `Assessment`: `total_project_score`, `total_project_impact`, `is_complete`, `potential_negative_impact`, `potential_positive_impact`, `environmental_setting`, `status`, `approved_by`, `recommendations`, أختام زمنية.
  - `MonitoringRecord`: `final_assessment`, أختام زمنية.

كل هذه الكيانات والحقول منعكسة في الرسم البياني المحدث، الجداول الموجزة، ونماذج Mongoose أدناه.
...
// Pre-populated data
[
  { job_title_id: 1, title_name: "Program Manager" },
  { job_title_id: 2, title_name: "Project Manager" },
  { job_title_id: 3, title_name: "Site Supervisor" },
  { job_title_id: 4, title_name: "Environmental Officer" },
  { job_title_id: 5, title_name: "Contractor" },
  { job_title_id: 6, title_name: "Consultant" },
  { job_title_id: 7, title_name: "Community Representative" },
];
```
...
| POST   | `/api/v1/attachments`       | رفع مرفق          |
...
9. ⏳ تفعيل حماية المسارات بحسب الأدوار (لاحقًا)
